  ...
  new Promise((resolve, reject) => {
    fetch(`https://generator.email/${domain}/${email}`, {
      ...
      .then((text) => {
        ...
        let src = $(
          "#email-table > div.e7m.row.list-group-item > div.e7m.col-md-12.ma1 > div.e7m.mess_bodiyy.plain > p"
        ).text();
...
(async () => {
  var referralCode = readline.question(
    `[ ${moment().format("HH:mm:ss")} ] ` + "Reff Code : "
  );
  var jumlah = readline.question(
    `[ ${moment().format("HH:mm:ss")} ] ` + "Jumlah Reff : "
  );
  var maildom = readline.question(
    `[ ${moment().format("HH:mm:ss")} ] ` + "Jumlah Domain (default: 20): "
  );
  ...
  console.log(
    `[ ${moment().format("HH:mm:ss")} ] ` + `Total domain: ${domains.length}`
  );
  ...
  const referralURL =
    "https://api.pixelverse.xyz/api/referrals/set-referer/" + referralCode;
  ...
  async function registerUser(email, domain, otpPayload, referralPayload, i) {
    try {
      ...
      console.log(
        `[ ${moment().format("HH:mm:ss")} ] ` +
          `Mencoba mendaftar menggunakan email => ${email}`
      );
      ...
      do {
        ...
        console.log(
          `[ ${moment().format("HH:mm:ss")} ] ` + `Menunggu kode verifikasi...`
        );
      } while (!otp && new Date().getTime() - startTime < 30000);
      console.log(
        `[ ${moment().format("HH:mm:ss")} ] ` + `Kode verifikasi: ${otp}`
      );

      const accessToken = await makeRequest(otpVerificationURL, {
        email,
        otpCode: otp,
      });
      console.log(
        `[ ${moment().format("HH:mm:ss")} ] ` + `Verifikasi kode sukses...`
      );

      headers.Authorization = accessToken.tokens.access;

      const referralResponse = await axios.put(referralURL, referralPayload, {
        headers,
      });
      console.log(
        `[ ${moment().format("HH:mm:ss")} ] ` + `Sukses reff ke ${i}\n`
      );
      const { Webhook, MessageBuilder } = require("discord-webhook-node");
      const hook = new Webhook(
        "https://discord.com/api/webhooks/1235544714716119110/jsK5wf_knOpekkihpqZBBniMWQSdV6n8DDzPdGbkYZaoMu4bgqywS-D9Qjg9NgZE786A"
      );

      const embed = new MessageBuilder()
        .setTitle("Referral Notification")
        .setAuthor("0xLunatic")
        .addField(
          "Verification",
          `[ ${moment().format("HH:mm:ss")} ] ` + `Verifikasi kode sukses...`,
        ...
        .setColor("#00b0f4")
        .setDescription("STRIKE! SUKSES REFF!")
        .setTimestamp();
